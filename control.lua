...
--!See maps.lua for available maps
local selection = ""
local maps = require("maps")
local map_setting = settings.global["comfyfactorio_map_selection"]
if map_setting and map_setting.value then
	selection = map_setting.value
end

if selection ~= "" and selection ~= "no map" then
	if selection and maps[selection] then
		require(maps[selection])
	end
else
	log("comfyfactorio ran with no scenario selected")
end
